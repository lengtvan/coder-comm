...
const slice= createSlice({
    ...
    reducers:{
        ...
        getFriendRequestSentSuccess(state,action){
            state.isLoading=false;
            state.error=null;
            const{users,count,totalPages}=action.payload;
            users.forEach((user)=>(state.usersById[user._id]=user));
...
export const getUsers=({filterName, page=1, limit=12})=>
async(dispatch)=>{
...
export const declineRequest=(targetUserId)=>async(dispatch)=>{
    ...
    try{
        ...
        dispatch(slice.actions.declineRequestSuccess({...response.data.data,targetUserId}));
...
export const acceptRequest=(targetUserId)=>async(dispatch)=>{
    ...
    try{
        ...
        dispatch(slice.actions.acceptRequestSuccess({...response.data.data,targetUserId}))
...
export const cancelRequest=(targetUserId)=>async(dispatch)=>{
    ...
    try{
        ...
        dispatch(slice.actions.cancelRequestSuccess({...response.data.data,targetUserId}))
...
export const removeFriend=(targetUserId)=>async(dispatch)=>{
    ...
    try{
        ...
        dispatch(slice.actions.removeFriendSuccess({...response.data.data,targetUserId}))
...
export const getFriends=({filterName, page=1, limit=12})=>
async(dispatch)=>{
    dispatch(slice.actions.startLoading());
    try{
        const params= {page,limit};
...
export const getFriendRequest=({filterName, page=1, limit=12})=>
async(dispatch)=>{
    dispatch(slice.actions.startLoading());
    try{
        const params= {page,limit};
...
export const getFriendRequestSent=({filterName, page=1,limit=12})=>
async(dispatch)=>{
    dispatch(slice.actions.startLoading())
    try{
        const params={page,limit};
        ...
        dispatch(slice.actions.getFriendRequestSentSuccess(response.data.data))
    }
    catch(error){
        dispatch(slice.actions.hasError(error.message))
